    ...
    function loan_application() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($product->maxmum_time >= $installment) {
                ...
                $installment_amount = $this->loanbase->get_installment($product->interest_rate, $amount, $installment, $product->interest_method, $product->interval, $product->id);

                $total_interest_amount = $this->loanbase->totalInterest($product->interest_rate, $amount, $installment, $installment_amount, $product->interest_method, $product->interval, $product->id);
                
                //custom staff loan
                if($product_id == 4){
                    if($installment > 12){                       
                      $createloan['installment_amount'] = round(($total_interest_amount + $amount)/$installment, 2);
                    } else {
...
$first_intall_date = format_date(trim($this->input->post('disbursedate')));
            ...
            if ($create) {
                //load data
                $up = array('disburse' => 1, 'loan_principle_account' => $this->input->post('loan_disburse_accountto'));
                $this->db->update('loan_contract', $up, array('LID' => $LID));
                ...
                if($product->interval == 3){
                   
                    //make yearly repayment schedule
                    $j = 1;  
                    $Sum_repayamount = 0;
                    $Sum_interest = 0;
                    $Sum_principle = 0;
                 ...
                 foreach ($schedule as $key => $values1) {
                   ...
                   $Sum_repayamount += $values1['repayamount'];
                   $Sum_interest += $values1['interest'];
                   $Sum_principle += $values1['principle'];
                   $Sum_balance = $values1['balance'];
                   ...
                   $array1['LID'] = $LID;
                   $array1['PIN'] = $PIN;
                   $array1['installment_number'] = $installment_number/12;
                    ...
                    if (count($open_repayment) > 0) {
                        $receipt = $this->loan_model->loan_repay_receipt($LID, $amount, $paydate);
                         ...
                         if ($unearned) {
                            ...
                            $repay_amount_install_original = $loaninfo->installment_amount;
