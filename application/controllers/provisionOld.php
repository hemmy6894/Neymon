    ...
    function provision_rate() {
       ...
       // print_r($result);
        ...
        foreach ($result as $key => $value) {
            ...
            $max_date = date('Y-m-d', strtotime('+' . ($product->warning_day + 1) . ' days', strtotime($value->early_date)));
              ...
              $outstanding_amount = $this->balance_outstanding_loan($value->LID);

            $prov_amount = (($provision->rate / 100) * $outstanding_amount);
             $prov_amount_remain = round(($prov_amount - $previous_amount), 2);
              ...
              //debit Provision for doubtfull
              $ledger['credit'] = 0;
              $ledger['debit'] = 0;
              $debit_account = 1050002;
