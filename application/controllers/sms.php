...
class SMS extends CI_Controller {
    ...
    function newcontact($id = null) {
     ...
     function loanreminder() {
        ...
        $this->load->library('pagination');
        $this->data['title'] = 'Loan Reminders';
        if (!$this->ion_auth->logged_in()) {
            //redirect them to the login page
            redirect('auth/login', 'refresh');
        }
        ...
        $config["base_url"] = site_url(current_lang() . '/sms/loanreminder/');
       $config["total_rows"] = $this->sms_model->count_loanreminders($key);
        $config["uri_segment"] = 4;
        ...
        $this->data['loanreminders'] = $this->sms_model->all_loanreminders($key, $config["per_page"], $page);

        $this->data['serial']  = $page; 


        $this->data['content'] = 'sms/loanreminder';
        $this->load->view('template', $this->data);
    }
