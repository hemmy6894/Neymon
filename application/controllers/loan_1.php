...
$first_intall_date = format_date(trim($this->input->post('disbursedate')));
            ...
            if ($create) {
                ...
                $schedule = $this->loanbase->create_repayment_schedule($infodata->installment_amount, $infodata->rate, $infodata->number_istallment, $first_intall_date, $infodata->basic_amount, $LID, $product->interest_method, $product->interval);
            ...
            if ($amount > 0) {
                if ($loaninfo->status == 4) {
                    ...
                    if (count($open_repayment) > 0) {
                        ...
                        //checking if there is balance for next installation
                         if($previous_remain_balance > 0){
                         //credit loan prepayment account
                         $prepaid = $this->loan_model->loan_prepayment_account('credit',$paydate, $LID, $received_account, $previous_remain_balance, $previous_remain_balance_id);
                        }
...
//exit;
                                        ...
                                        $previous_remain_balance_id = $this->loan_model->get_previous_remain_balance_id($LID);
                                        $prepaid = $this->loan_model->loan_prepayment_account('debit',$paydate, $LID, $received_account, round($amount_tmp, 2), $previous_remain_balance_id);                      
                                        break;
                                    }
                                }
                            } else {
