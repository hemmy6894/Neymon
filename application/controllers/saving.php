...
class Saving extends CI_Controller {
    ...
    function autosuggest($id) {
        ...
        if (!$q)
            return;
        $auto = $this->db->query("SELECT PID,firstname, middlename, lastname,member_id,category FROM members WHERE PIN='$pin' AND ( PID LIKE '$q%' OR  member_id LIKE '$q%' OR firstname LIKE '$q%' OR lastname LIKE '$q%')")->result();
    ...
    function search_member() {
        ...
        echo $column = $this->input->post('column');
        $explode = explode('-', $value);
        $value = $explode[0];
        $pid = null;
        $member_id = null;
