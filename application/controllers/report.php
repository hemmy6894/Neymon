...
class Report extends CI_Controller {
    ...
    function create_ledger_trans_title($link, $id = null) {
        $this->data['other_years'] = $this->finance_model->financial_years(); 
        ...
        if (!is_null($id)) {
            $this->data['reportinfo'] = $this->report_model->report_list($id)->row();
             //$query3=$this->db->get('report_table')->where('id', $id);
                       $ac= $this->data['reportinfo']->account;
                       $year = $this->data['reportinfo']->year;
            //$this->data['account_list2e'] = $this->finance_model->account_chart_hiarchy(array(10 => array(1,2,3,4,5), 20 => array(1, 2)));
                 if($ac != 'ALL'){
                 $this->data['account_list2e'] = $this->finance_model->account_chart(null, $ac)->row();
                 $this->data['account_name'] = $this->data['account_list2e']->name;
                 $this->data['account_no'] = $this->data['account_list2e']->account;
                 }
                 else {
                   $this->data['account_name'] = "ALL";
                   $this->data['account_no'] = "ALL"; 
    ...
    function ledger_trans_view($link, $id) {
        ...
        if ($reportinfo->account == 'ALL'){ $ac = null; } else { $ac = $reportinfo->account;}
        $year = $reportinfo->year;
        $this->data['transaction'] = $this->report_model->ledger_trans($reportinfo->fromdate, $reportinfo->todate, $ac, $year);
