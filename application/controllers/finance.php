...
class Finance extends CI_Controller {
    ...
    function close_open_year() {
        $this->data['title'] = 'Close Previous Year, Open New Year and Add Opening Balance in New Year';
        $this->form_validation->set_rules('year', 'New Year', 'required');
        $this->form_validation->set_rules('date', 'New Year Start Date', 'required|valid_date');

        if ($this->form_validation->run() == TRUE) {
            $new_year = $this->input->post('year');
            $start_date = $this->input->post('date');
            $check = $this->db->get_where('year', array('year' => $new_year))->row();
            if (!$check) {
                $account_credit = $this->input->post('opening_credit');
                $account_debit = $this->input->post('opening_debit');
                $this->db->update('year', array('status' => 0));
                $this->db->insert('year', array('status' => 1, 'year' => $new_year));
                ...
                foreach ($account_credit as $key => $value) {
                    $credit = str_replace(',', '', $value);
                    $debit = str_replace(',', '', $account_debit[$key]);
                    if($credit<> '' || $debit <> ''){
                ...
                $this->session->set_flashdata('message', 'New Financial Year Recorded');
                    redirect(current_lang() . '/finance/close_open_year', 'refresh');
            } else {
                $this->data['warning'] = 'The New Year ' . $new_year . ' already exist in the system, You can not open it again';
            }
        }
        $this->data['account_chart'] = $this->finance_model->account_chart()->result();
        $this->data['content'] = 'finance/close_open_year';
        $this->load->view('template', $this->data);
    }

    function close_year($id = null) {

        if ($id != null) {
            $id = decode_id($id);
            $this->finance_model->close_year($id);
            redirect(current_lang() . '/finance/close_year');
        }

        $this->data['title'] = 'Close Year';
        $this->data['years'] = $this->finance_model->financial_years_all();
        $this->data['content'] = 'finance/close_year';
        $this->load->view('template', $this->data);
    }
