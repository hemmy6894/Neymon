    ...
    <table class="table table-bordered table-striped">
        <thead>
            <tr>
                ...
                <th><?php echo ($link_cat == 1 ? 'Member Joined From' : (($link_cat ==3) ? 'Disbursed From' :($link_cat == 2 ? 'Disbursed Until' :  'From'))); ?></th>
                <?php if($link_cat != 2){ ?>
            ...
            <?php foreach ($reportlist as $key => $value) { ?>
                <tr>
                    <td style="width: 300px;"><?php echo anchor(current_lang() . "/report_loan/create_loan_report_title/" . $link_cat . '/' . encode_id($value->id), ' <i class="fa fa-edit"></i> ' . lang('button_edit')); ?>   &nbsp; | &nbsp;
                        ...
                        echo anchor(current_lang() . "/report_loan/loan_arrears_view/" . $link_cat . '/' . encode_id($value->id), ' <i class="fa fa-eye"></i> ' . lang('button_view'));    
                        }else if($link_cat == 7) {
                        echo anchor(current_lang() . "/report_loan/loan_maturity_view/" . $link_cat . '/' . encode_id($value->id), ' <i class="fa fa-eye"></i> ' . lang('button_view'));    
                        }else if($link_cat == 7) {
                        echo anchor(current_lang() . "/report_loan/loan_classfication_view/" . $link_cat . '/' . encode_id($value->id), ' <i class="fa fa-eye"></i> ' . lang('button_view'));    
                        }
                        ?></td>
                    <?php if($link_cat != 2){ ?>
