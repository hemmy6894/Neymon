...
class Loan_Model extends CI_Model {
    ...
    function loan_disburse_date($loanid) {
        $sql = "SELECT loan_contract_repayment_schedule.* FROM loan_contract_repayment_schedule WHERE loan_contract_repayment_schedule.LID='$loanid' order by repaydate ASC LIMIT  1 ";
        return $this->db->query($sql)->row()->repaydate;
    }
    ...
    function loan_wait_evaluation() {
        $pin = current_user()->PIN;
        return $this->db->query("SELECT * FROM loan_contract WHERE PIN='$pin' AND (status=0 OR status=3) AND push_to_evaluation=1 ORDER BY applicationdate DESC")->result();
    }
    ...
    function loan_wait_disburse() {
        $pin = current_user()->PIN;
        return $this->db->query("SELECT * FROM loan_contract WHERE PIN='$pin' AND status=4 AND (disburse=0 OR disburse=1) AND multiple_disburse_status !=2 ORDER BY applicationdate DESC")->result();
    }
    ...
    function loan_repay_receipt($LID, $amount, $paydate, $paymethod, $check_number_received) {
        $pin = current_user()->PIN;
        $receipt = $this->receiptNo();
        $receiptcustom = $this->receiptCustom();

        $array = array(
            'LID' => $LID,
            'receipt' => $receipt,
            'customer_receipt' => $receiptcustom,
            'amount' => $amount,
            'paydate' => $paydate,
            'paymethod' => $paymethod,
    ...
    function get_previous_receipt($LID, $PIN) {
        $this->db->where('PIN', $PIN);
        $this->db->where('LID', $LID);
        $val = $this->db->order_by('id', 'DESC')->limit(1)->get('loan_repayment_receipt')->row();
        if ($val) {
            return $val->receipt;
    ...
    function get_previous_receipt_date($LID, $PIN) {
        $this->db->where('PIN', $PIN);
        $this->db->where('LID', $LID);
        $val = $this->db->order_by('id', 'DESC')->limit(1)->get('loan_repayment_receipt')->row();
        if ($val) {
    ...
    function record_loan_repayment($array_data, $repay_schedule_ref, $received_in) {
        ...
        if($array_data['amount'] > 0){
            ...
            $ledger['debit'] = $array_data['amount'];
        ...
        if($array_data['principle'] > 0){
            $ledger['description'] = "Loan Repayment Principle";
        ...
        if($array_data['interest'] > 0){
            $ledger['description'] = "Loan Repayment Interest";
            ...
            if ($unearned) {
                ...
                $ledger['credit'] = 0;
                $ledger['debit'] = 0;
                $ledger['account'] = $product->loan_interest_receivable_account;
                //$ledger['account'] = $infodata->loan_principle_account;
        ...
        if (array_key_exists('penalt', $array_data)) {
            if($array_data['penalt'] > 0){
                $ledger['description'] = "Loan Repayment Penalty";
    ...
    function loan_prepayment_account_before_accrualrun($paydate, $LID, $received_in, $previous_remain_balance, $previous_remain_balance_id ){
        $pin = current_user()->PIN;
        //general entry id
        $ledger_entry = array('date' => $paydate, 'PIN' => $pin);
        $this->db->insert('general_ledger_entry', $ledger_entry);
        ...
        $ledger = array(
            ...
            'LID' => $LID,
            'date' => $paydate,
            'description' => 'Balance For The Next Installment M',
            'linkto' => 'loan_balance_carry.id',
            'fromtable' => 'loan_balance_carry',
            'paid' => 0,
            'year' => active_year(),
    ...
    //when accruall is run and the amount is insuffiecent to installment
    function record_loan_repayment_insufficient($array_data, $repay_schedule_ref, $received_in, $insufficient_amount) {
        $pin = current_user()->PIN;
        $this->db->trans_start();
        $insert = $this->db->insert('loan_contract_repayment', $array_data);
        $referenceID = $this->db->insert_id();
        ...
        $this->db->update('loan_contract_repayment_schedule', array('status' => 0, 'unearned' => 1, 'insufficient_paid'=>$insufficient_amount), array('id' => $repay_schedule_ref));
        $this->db->update('loan_repayment_receipt', array('affect_loan' => 1, 'installment' => $array_data['installment']), array('receipt' => $array_data['receipt']));
        $this->db->trans_complete();
        return $insert;
    }
    ...
    function receiptCustom() {
        $query = $this->db->query("SELECT customreceipt  FROM auto_inc")->row();
        $query1 = $this->db->query("UPDATE auto_inc SET customreceipt=customreceipt+1 ");
        return "TEA/".$query->customreceipt;
    }
    ...
    function loan_holder_name($LID) {
        $sql = "SELECT CONCAT(COALESCE(members.firstname, ''),' ',COALESCE(members.middlename, ''),' ',COALESCE(members.lastname, '')) as name FROM members INNER JOIN loan_contract ON members.PID=loan_contract.PID WHERE loan_contract.LID='$LID'";
    ...
    function installment_paid_amount($LID, $installment) {

        $sql = "SELECT SUM(amount) as tamount, SUM(interest) as interest_paid, SUM(principle) as principle_paid, SUM(penalt) as penalt_paid, SUM(balance) as balance_remain FROM loan_contract_repayment  WHERE LID='$LID' AND installment='$installment' ";
        return $this->db->query($sql)->result();
    }

    function already_disbursed_amount($LID) {
        $sql = "SELECT SUM(disburseamount) as sumdisburseamount FROM loan_contract_disburse  WHERE LID='$LID' ";
    ...
    function numbertoword($number){
        ...
        return $this->numbertowords->convert_number_with_points($number);
