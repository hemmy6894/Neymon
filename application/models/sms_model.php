...
class Sms_Model extends CI_Model {
    ...
    function group_number($gp) {
     ...
     function count_loanreminders($key = null) {
        $pin = current_user()->PIN;
        $sql = " SELECT * FROM sms_sent WHERE PIN='$pin'  ";
        if (!is_null($key)) {
            $sql .= " AND (mobile LIKE '%$key%' OR delivery_status LIKE '%$key%' OR sender LIKE '%$key%' OR
            message_id LIKE '%$key%' )";
        }
    ...
    function all_loanreminders($key, $limit, $start) {
        $pin = current_user()->PIN;
        $sql = " SELECT * FROM sms_sent WHERE PIN='$pin' ";
        if (!is_null($key)) {
            $sql .= " AND (mobile LIKE '%$key%' OR delivery_status LIKE '%$key%' OR sender LIKE '%$key%' OR
